  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <Card title="Colonne cible (à prédire)">
        <select value={targetColumn || ""} onChange={(e) => handleTargetChange(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-rose-500">
        ...
        {targetColumn && taskType && (
          <div className="mt-3 flex items-center gap-3">
            <span className={`text-sm px-3 py-1 rounded-full ${taskType === "classification" ? "bg-violet-100 text-violet-700" : "bg-green-100 text-green-700"}`}>
              {taskType === "classification" ? "Classification" : "Régression"}
            </span>
            <button onClick={() => setTaskType(taskType === "classification" ? "regression" : "classification")} className="text-xs text-gray-500 hover:text-rose-600 underline">Changer manuellement</button>
          </div>
        )}
      </Card>
      {targetColumn && (
        <Card title={`Features (${featureColumns.length}/${availableFeatures.length} sélectionnées)`}>
                ...
                {availableFeatures.map((col) => (
                  <tr key={col} className={`border-b border-gray-50 hover:bg-gray-50 cursor-pointer ${featureColumns.includes(col) ? "" : "opacity-50"}`} onClick={() => toggleFeature(col)}>
                    ...
                    <td className="px-3 py-2">
                      ...
                      {descriptions[col] && (
                        <div className="text-xs text-rose-600 mt-0.5">🤖 {descriptions[col]}</div>
                      )}
                      {loadingDescriptions && !descriptions[col] && (
                        <div className="text-xs text-gray-500 mt-0.5 animate-pulse">Analyse IA...</div>
                      )}
                    </td>
                    <td className="px-3 py-2">
                    ...
                    <td className="px-3 py-2">
                      {correlations[col] !== undefined ? <CorrelationBar value={correlations[col]} /> : <span className="text-xs text-gray-500">—</span>}
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="mt-3 flex items-center justify-between">
            <button onClick={toggleAll} className="text-xs text-rose-600 hover:text-rose-800 underline">
